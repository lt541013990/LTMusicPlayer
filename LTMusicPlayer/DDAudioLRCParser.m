...
+ (DDAudioLRC *)parser:(NSString *)lrc
{
        ...
        for (NSString *tag in tags) {
            if (![tmp metadataForKey:tag]) {
                ...
                if ([aLine hasPrefix:prefix] && [aLine hasSuffix:@"]"]) {
                    ...
                    if ([tag isEqualToString:(NSString *)kDDLRCMetadataKeyTIME]) {
                        NSInteger minute = [[[info componentsSeparatedByString:@":"].firstObject stringByReplacingOccurrencesOfString:@"(" withString:@""] integerValue];
                        NSInteger second = [[[info componentsSeparatedByString:@":"].lastObject stringByReplacingOccurrencesOfString:@")" withString:@""] integerValue];
                        info = [NSString stringWithFormat:@"%ld", (minute * 60 + second)];
                    }
    ...
    NSArray *unitArray = [self sortedLRCUnits:units];
    for (NSInteger i = 0; i < unitArray.count - 1; i++) {
        DDAudioLRCUnit *curUnit = unitArray[i];
        DDAudioLRCUnit *nextUnit = unitArray[i+1];
        
        [curUnit setValue:@(nextUnit.sec - curUnit.sec) forKey:@"timeLength"];
    }
    
    [unitArray.lastObject setValue:@([[tmp metadataForKey:(NSString *)kDDLRCMetadataKeyTIME] floatValue] - [[unitArray.lastObject valueForKey:@"sec"] floatValue]) forKey:@"timeLength"];
